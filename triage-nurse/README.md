...
> Five patients in a waiting room. Classify each into one of the five Manchester / KTAS triage levels. Ground truth from a panel of three triage experts (`dataset/emergency-triage.csv`, n=1267). Built as an OpenReward environment whose rollouts feed the Triage Lab UI in the parent repo.
...
src/triage_nurse/
  ...
  dataset.py         CSV loader + trajectory synthesis    [full]
...
tests/               smoke + dataset + scoring + cost     (37 tests)
```
...
- **Real ground truth.** `KTAS_expert` from a 3-expert-panel-graded dataset, no rubric.
...
- **Authoring goes away.** Every CSV row is a free task — `select_diverse_batch(seed=42)` returns 5 KTAS-diverse row indices, the env loads them as Patients.
...
Three demo batches pre-seeded (seeds 42, 7, 99). Smoke test on `batch-seed42` with gpt-5-mini: composite **1.000**, ordering bonus +0.10, 5 turns, £0.0043. The agent classified all five patients correctly first try and triaged the most acute first.

Pull the leaderboard (3 seeds × N models) by running each batch with each `--model`; cost ~£0.005/run.
